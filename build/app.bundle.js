...
// Remove post event
_ui.ui.postWrapper.addEventListener('click', removePost);

// Update post event
_ui.ui.postWrapper.addEventListener('click', enableEditState);

// Cancel update post event
_ui.ui.wrapper.addEventListener('click', cancelUpdate);

// Show post method:
function getPosts() {
  _easyHTTP.http.get('http://localhost:3000/posts').then(function (posts) {
    return _ui.ui.showPosts(posts);
  }).catch(function (err) {
...
function submitPost(e) {
  var data = {
    title: _ui.ui.postTitle.value,
    description: _ui.ui.postDesc.value
  };

  var id = _ui.ui.postID.value;
  ...
  if (data.title !== '' && data.description !== '') {
    if (id === '') {
      ...
      // Show alert
      _ui.ui.showAlert('Post added!', 'success', 'afterend');
    } else {
      // Update post
      _easyHTTP.http.put('http://localhost:3000/posts/' + id, data).then(function () {
        _ui.ui.showAlert('Post has been updated!', 'success', 'afterend');
    ...
    // Show alert
    _ui.ui.showAlert('Please fill in all fields', 'danger', 'beforebegin');
  }
...
function removePost(e) {
  ...
  if (targetID === 'post-remove') {
    ...
    if (confirm('Are you sure?')) {
      _easyHTTP.http.delete('http://localhost:3000/posts/' + id).then(function () {
        getPosts();
        _ui.ui.showAlert('Post has been removed!', 'success', 'afterend');
      });
    }
  }
...
function enableEditState(e) {
  ...
  if (targetID === 'post-edit') {
    var id = e.target.parentElement.parentElement.dataset.id;
    var currPostTitle = e.target.parentElement.parentElement.parentElement.previousElementSibling.previousElementSibling.textContent;
    var currPostDesc = e.target.parentElement.parentElement.parentElement.previousElementSibling.textContent;
...
var UI = function () {
  ...
  _createClass(UI, [{
    ...
    key: 'showAlert',
    value: function showAlert(msg, type, position) {
      this.clearAlert();
      var alerUI = document.createElement('div');
      alerUI.id = 'alert';
    ...
    value: function clearAlert() {
      var currAlert = document.getElementById('alert');
      if (currAlert) {
        currAlert.remove();
      }
    }
